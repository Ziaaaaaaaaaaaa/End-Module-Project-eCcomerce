...
let card = document.querySelector('#products-card');
  ...
  arr.forEach((data) => {
    ...
    <div class="card" style="width: 14rem; ">
        <img src="${data.img}" class="card-img-top p-3" alt="">
    <div class="card-body">
          <h5 class="card-title">${data.make}</h5>
          <p class="card-text text-black">${data.specs}</p>
          <p class="card-text">R${data.Amount}</p>
          <button class="btn btn-danger" onclick='addToCheckout(${JSON.stringify(data)})'>Buy Now</button>
        </div>
        </div>
      `;
  });
